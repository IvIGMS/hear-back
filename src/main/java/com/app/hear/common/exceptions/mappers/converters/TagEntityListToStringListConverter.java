...
public class TagEntityListToStringListConverter
    implements Converter<List<TagEntity>, List<String>> {
  ...
  public List<String> convert(MappingContext<List<TagEntity>, List<String>> context) {
    ...
    return context.getSource().stream().map(TagEntity::getName).collect(Collectors.toList());
  }
}
